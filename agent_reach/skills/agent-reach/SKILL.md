...
7. Use `agent-reach plan candidates` when a saved ledger needs no-model URL or ID dedupe.
8. Use diagnostic hints only to explain provenance or extraction shape; downstream code owns ranking and selection.
9. Use `AgentReachClient` only when the host Python environment has Agent Reach installed into it directly.
10. Choose channels from the live `channels --json` contract for the user's task; Agent Reach does not own ranking, routing, or source policy.
11. Choose advanced collection controls such as page, cursor, and time-window options from the live `operation_contracts`; Agent Reach does not choose collection scope for you.
12. Treat `scout` as a planning helper and capability snapshot, not the default route for a lightweight ask.
13. Treat large-scale research as explicit opt-in. Keep narrow asks on `collect --json` unless the caller clearly wants a broader run.
14. Use `searxng` only after `searxng-base-url` is configured or `SEARXNG_BASE_URL` is set.
15. Use `crawl4ai` only when the optional extra and browser runtime are available; `crawl` requires an explicit `--query` goal.
16. Use `reddit` through `rdt-cli`; it does not need Reddit OAuth, client credentials, or a User-Agent config.
17. Treat Twitter/X as opt-in and cookie-based; authenticated-but-unprobed `warn` means collect may work, but operation readiness is unverified.
18. In arbitrary downstream repositories, use the globally installed `agent-reach` CLI. Do not require copying Agent Reach repo files into the downstream project unless the user explicitly asks for repo-local plugin artifacts.
...
9. Use `web read` for selected URLs, not every search result.
10. Inspect source hints and web hygiene only as non-authoritative diagnostics.
11. Return partial results with clear channel failures instead of blocking on one non-required backend.
